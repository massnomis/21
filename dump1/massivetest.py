...
while qqq < 10:
    ...
    inputfixed = 0.1
    df.loc[i] = [i,inputfixed,amount_toToken_correct_eth,amount_toToken_correct_op,amount_toToken_correct_BSC,amount_toToken_correct_GNOSIS, amount_toToken_correct_POLY, amount_toToken_correct_ARB,amount_toToken_correct_AVAX,bestoutput]
    ...
    token_return = json.loads(token_return.text)
    amount_toToken = int(token_return['toTokenAmount'])
    inverse_bsc = ((amount_toToken) / (10 ** 6))
    ...
    df2.loc[i] = [i, bestoutputxxx, inverse_eth, inverse_op, inverse_bsc,
                 inverse_gnosis, inverse_poly, inverse_arb,
