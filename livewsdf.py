...
async def consumer() -> None:
    async with websockets.connect("wss://ftx.com/ws/") as websocket:
        await websocket.send(
            json.dumps(
                {"op": "subscribe", "channel": "orderbook", "market": "ETH-PERP"}
            )
        )
            ...
            with placeholder1.container():
                ...
                if message["type"] == "update":
                    ...
                    for i in range(len(asks_update)):
                        ...
                        asks.append(asks_update.iloc[i])
                    ...
                    for i in range(len(bids_update)):
                        ...
                        bids.append(bids_update.iloc[i])
                    ...
                    fig = make_subplots(specs=[[{"secondary_y": True}]])
                    fig.add_trace(go.Bar(x=asks['price_ask'], y=asks['size_ask'], name="asks"),secondary_y=False,)
