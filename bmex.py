...
ask_new['mm_ask_price']  = round(round(asks['price_ask'] * ((1-((1*((1-(latest_rateAPY_spot*pct_expiry_dated))))*apy_to_beat))*(1*((1-(latest_rateAPY_spot*pct_expiry_dated))))) / precision_price) * precision_price, -int(math.floor(math.log10(precision_price)))) + 400
...
st.plotly_chart(fig, use_container_width=True)
fig = make_subplots(specs=[[{"secondary_y": True}]])
fig.add_trace(go.Scatter(x=ask_new['mm_ask_price'], y=ask_new['mm_ask_size'], name="asks"),secondary_y=True,)
fig.add_trace(go.Scatter(x=bid_new['mm_bid_price'], y=bid_new['mm_bid_size'], name="bids"),secondary_y=True,)
fig.update_layout(title_text="orderbook")
st.plotly_chart(fig, use_container_width=True)
